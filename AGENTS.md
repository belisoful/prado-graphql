# PRADO GraphQL Extension Agent Guidelines
...
- Extension Exceptions use errorCodes specified in src/errorMessages.txt; messages.txt is purely for user information display only.
...
- The current version is 1.0.0. git HEAD is working on version 1.0.1.

## GraphQL-Specific Guidelines

### TGraphQLService
- `TGraphQLService` extends `\Prado\Web\Services\TService` directly — NOT `TRpcService`.
- The dispatch model is document → resolver graph, NOT method → callable.
- Schema is built via the `IGraphQLSchemaBuilder` interface; the builder class is set via `SchemaBuilderClass`.
- Request parsing supports `application/json`, `application/graphql`, and `multipart/form-data`.
- Batched queries (array of operations) are opt-in via `EnableBatchedQueries`.
- Introspection can be disabled via `EnableIntrospection = false` (adds `DisableIntrospection` validation rule).
- Query depth and complexity limits are enforced via webonyx/graphql-php validation rules.
- Auth context is injected via `TGraphQLContext` which threads `TApplication`, `THttpRequest`, and the active `IUser` through the resolver graph.
- Error formatting maps PRADO exceptions to `GraphQL\Error\Error` instances via a configurable formatter.
- Persisted queries (APQ) are cached by hash via the PRADO cache module identified by `CacheID`.

### IGraphQLSchemaBuilder
- Implement `buildSchema(TGraphQLService $service): \GraphQL\Type\Schema`.
- The schema is built once per request and held in the service instance.

### TGraphQLContext
- Passed as `$contextValue` to `GraphQL::executeQuery()`.
- Provides access to the application, request, response, and authenticated user.
- Resolvers should type-hint `TGraphQLContext` on the `$context` parameter.

### TGraphQLServiceConfig
- Optional application module (`<module class="Prado\Web\Services\TGraphQLServiceConfig">`) that provides
  shared defaults consumed by one or more `TGraphQLService` instances via their `ConfigID` property.
- Acts as the Composer bootstrap class (`extra.bootstrap`).
...
- Optional extensions for additional features: apcu, mbstring, openssl, zlib
